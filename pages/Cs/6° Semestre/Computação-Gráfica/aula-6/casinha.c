...
float posx=0, posy=20, posz=50; //Variáveis que definem a posição da câmera
...
void drawHouse(int x, int y, int z, int size) {
  ...
  float sky_r = fmin(0.5 + 0.5 * cos(time_of_day - M_2_PI / 2), 0.4 );
  float sky_g = fmin(0.7 + 0.3 * cos(time_of_day - M_2_PI / 2), 0.7);
  float sky_b = fmin(1.0 + 0.0 * cos(time_of_day - M_2_PI / 2), 0.9 );
